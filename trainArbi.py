...
from deeplab_resnet import DeepLabResNetModel, ImageReader, decode_labels, inv_preprocess, prepare_label, DeepLabResNetModelarbiUp

IMG_MEAN = np.array((104.00698793, 116.66876762, 122.67891434), dtype=np.float32)

BATCH_SIZE = 10
DATA_DIRECTORY = '/home/vpa/Datasets/VOCdevkit2012'
...
SNAPSHOT_DIR = './snapshots/depthwiseUp3x3conv3/'
WEIGHT_DECAY = 0.0005
...
def main():
    ...
    # Create network.
    net = DeepLabResNetModelarbiUp({'data': image_batch}, is_training=args.is_training, num_classes=args.num_classes)
    ...
    # Predictions.
    raw_output = net.layers['transpose_back_conv']
    # Which variables to load. Running means and variances are not trainable,
    # thus all_variables() should be restored.
    restore_var = [v for v in tf.global_variables() if 'transpose' not in v.name]
    ...
    if args.restore_from is not None:
        ...
        checkpoint = tf.train.get_checkpoint_state(args.snapshot_dir)
