...
RESTORE_FROM = './deeplab_resnet/deeplab_resnet.ckpt'
SAVE_DIR = './output/test50msc/'
...
def main():
    ...
    for step in range(args.num_steps):
        ...
        preds = sess.run(pred)
        once_end = time.time()
        print('image %d, net forward cost %d s' % (step+1, once_end-once_start))
        ...
        print('image %d, postprocessing cost %d s' % (step + 1, end - once_start))
