...
# from deeplab_resnet import structured_attention_before_softmax_DeepLabResNetModel, ImageReader, decode_labels, inv_preprocess, prepare_label
from deeplab_resnet import *

IMG_MEAN = np.array((104.00698793, 116.66876762, 122.67891434), dtype=np.float32)

BATCH_SIZE = 5
DATA_DIRECTORY = '/media/E/Datasets/VOCdevkit2012/VOC2012'
...
def val():
    ...
    with tf.variable_scope(name_or_scope='', reuse=tf.AUTO_REUSE):
        ...
        # Create network.
        net = structured_attention_finalscale_before_softmax_DeepLabResNetModel({'data': image_batch}, is_training=False,
...
def main():
    ...
    # Create network.
    # net = structured_attention_DeepLabResNetModel({'data': image_batch}, is_training=args.is_training, num_classes=args.num_classes)
    net = structured_attention_finalscale_before_softmax_DeepLabResNetModel({'data': image_batch}, is_training=args.is_training,
                                                                num_classes=args.num_classes)
    ...
    restore_var = [v for v in tf.global_variables() if 'fc' not in v.name and 'decoder' not in v.name]
    all_trainable = [v for v in tf.trainable_variables() if 'beta' not in v.name and 'gamma' not in v.name]
    fc_trainable = [v for v in all_trainable if 'fc' in v.name or 'decoder' in v.name]
    conv_trainable = [v for v in all_trainable if 'fc' not in v.name and 'decoder' not in v.name]  # lr * 1.0
    ...
    # raw_prediction1 = tf.reshape(raw_output1, [-1, args.num_classes])
    raw_prediction2 = tf.reshape(raw_output2, [-1, args.num_classes])
    # raw_prediction3 = tf.reshape(raw_output3, [-1, args.num_classes])
    ...
    # prediction3 = tf.gather(raw_prediction3, indices)
    ...
    loss_sigmoid2 = tf.nn.sigmoid_cross_entropy_with_logits(labels=one_hot_gt, logits=prediction2)
    # loss_sigmoid3 = tf.nn.sigmoid_cross_entropy_with_logits(labels=one_hot_gt, logits=prediction3)

    l2_losses = [args.weight_decay * tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'weights' in v.name]
    # ls1 = tf.reduce_mean(loss_sigmoid1)
    ls2 = tf.reduce_mean(loss_sigmoid2)
    # ls3 = tf.reduce_mean(loss_sigmoid3)
    ls = tf.reduce_mean(loss)
    l2_ls = tf.add_n(l2_losses)
    # reduced_loss = tf.reduce_mean(loss_sigmoid1) + tf.reduce_mean(loss_sigmoid2) + tf.reduce_mean(loss) + tf.add_n(l2_losses)
    # reduced_loss = ls1+ls2+ls3+ls+l2_ls
    loss_list = [ls2, ls, l2_ls]
    loss_weight = [1.0, 1.0, 1.0]

    reduced_loss = tf.tensordot(loss_list,loss_weight,1)


    # loss_sigmoid1_summary = tf.summary.scalar('loss_sigmoid1', ls1)
    loss_sigmoid2_summary = tf.summary.scalar('loss_sigmoid2', ls2)
    # loss_sigmoid3_summary = tf.summary.scalar('loss_sigmoid3', ls3)
    loss_softmax_summary = tf.summary.scalar('loss_softmax', ls)
    loss_l2_losses_summary = tf.summary.scalar('l2_losses', l2_ls)
