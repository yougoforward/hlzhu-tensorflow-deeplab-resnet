...
#from misc import Network
from kaffe.tensorflow import Network
import tensorflow as tf

class DeepLabResNetModelEdgeAttention(Network):
    def setup(self, is_training, num_classes):
        ...
        # edge branch
        (self.feed('data')
             .sobel_layer(method='gray', name='sobel')
             .conv(7, 7, 64, 1, 1, biased=False, relu=False, padding='SAME', name='sobel_conv1')
             .batch_normalization(is_training=is_training, activation_fn=tf.nn.relu, name='sobel_bn1')
             .max_pool(3, 3, 2, 2, name='sobel_pool1'))
        # feature map 161 x 161 & add to conv1

        (self.feed('sobel_pool1')
             .conv(7, 7, 256, 1, 1, biased=False, relu=False, padding='SAME', name='sobel_conv2')
             .batch_normalization(is_training=is_training, activation_fn=tf.nn.relu, name='sobel_bn2')
             .max_pool(3, 3, 2, 2, name='sobel_pool2'))
        # feature map 80 x 80 & add to res2c

        (self.feed('sobel_pool2')
             .conv(7, 7, 512, 1, 1, biased=False, relu=False, padding='SAME', name='sobel_conv3')
             .batch_normalization(is_training=is_training, activation_fn=tf.nn.relu, name='sobel_bn3')
             .max_pool(3, 3, 2, 2, name='sobel_pool3'))
        # feature map 40 x 40 & add to res3d
        # end edge branch
        ...
        (self.feed('bn_conv1',
                   'sobel_pool1')
             .multiply(name='multiply_1'))

        (self.feed('multiply_1')
        ...
        '''change to fit the multiply the feature map with edge feature'''
        ...
        (self.feed('res2c_relu',
                   'sobel_pool2')
             .multiply(name='multiplu_2'))

        (self.feed('multiplu_2')
             .conv(1, 1, 512, 2, 2, biased=False, relu=False, name='res3a_branch1')
             .batch_normalization(is_training=is_training, activation_fn=None, name='bn3a_branch1'))
        ...
        (self.feed('res3b3_relu',
                   'sobel_pool3')
             .multiply(name='multiply_3'))

        (self.feed('multiply_3')
